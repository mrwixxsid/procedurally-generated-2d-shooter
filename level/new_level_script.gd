...
# --- SAFETY LIMITS ---
var MAX_ROOM_ATTEMPTS = 50
var MAX_TOTAL_STEPS = 10000
...
func reset():
	_step_history_set.clear()
	step_history.clear()
	rooms.clear()
	step_since_turn = 0
	last_room_position = Vector2.ZERO
...
func walk(steps_count):
	...
	var steps = min(steps_count, MAX_TOTAL_STEPS)
	...
	var attempts = 0
	while rooms.size() < min_rooms_required and attempts < MAX_ROOM_ATTEMPTS:
		attempts += 1
...
func place_room(pos):
	...
	var size_x = randi() % (max_room_size - min_room_size + 1) + min_room_size
	var size_y = randi() % (max_room_size - min_room_size + 1) + min_room_size
...
func is_area_free(center_pos, size):
	var top_left = (center_pos - size / 2).floor()
	var max_fill = int(size.x * size.y * 0.2)
	var filled = 0
...
func _carve_rect(center_pos, size):
	...
	for y in range(size.y):
		for x in range(size.x):
			...
			_step_history_set[pos] = true


# --- CORRIDOR ---
func _carve_corridor(center_pos):
